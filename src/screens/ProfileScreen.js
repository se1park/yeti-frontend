...
export function ProfileScreen({ authError, goTo, onLogout, onRefreshPlan, plan, session }) {
  const [subscribeVisible, setSubscribeVisible] = useState(false);
  const user = session?.user || {};
  const nickname = user.nickname || user.username || '유진';
  const username = user.username || 'yujin';
  const planName = plan?.name || plan?.plan || plan?.type || '체험 중';
  const planPrice = plan?.price || plan?.amount || '30,000원';
    ...
    <View style={styles.flex}>
      <Screen title="마이" right="설정">
        <Card style={styles.profile}>
          <Avatar label={nickname.slice(0, 2).toUpperCase()} color={BLUE} size={56} />
          <View style={styles.profileText}>
            <Text style={styles.name}>{nickname} <Text style={styles.handle}>@{username}</Text></Text>
        ...
        <Card style={styles.premium}>
          <Pill>{planName}</Pill>
          <Text style={styles.premiumTitle}>{plan?.title || '프리미엄 체험 5일 남음'}</Text>
          <Text style={styles.premiumText}>{plan?.description || 'AI 고도화, 그룹 즉시 적용, 학습 노트 요약을 계속 사용할 수 있어요.'}</Text>
          <Pressable onPress={() => setSubscribeVisible(true)} style={styles.whiteCta}>
            <Text style={styles.whiteCtaText}>연 {planPrice}으로 시작하기 ›</Text>
          </Pressable>
        </Card>
        ...
        <Card style={styles.menuCard}>
          ...
          <Menu label="로그아웃" value="" onPress={onLogout} danger />
        </Card>
      </Screen>
